...
import { Loader2, AlertCircle, CheckCircle2, XCircle, Clock, Filter, Search, ChevronDown } from 'lucide-react';
...
export default function AdminCommissionPage() {
  ...
  const fetchCommissions = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClient.get('/api/commissions', { params });
  ...
  const handleAction = async (id, action) => {
    setActionLoading(id);
    try {
      await apiClient.patch(`/api/commissions/${id}/${action}`);
      await fetchCommissions();
    } catch (err) {
      alert(err?.response?.data?.detail || `Fehler beim ${action === 'approve' ? 'Freigeben' : 'Stornieren'}`);
    }
    ...
    <div className="p-6 space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center" style={{ color: 'var(--color-danger)' }}>
          ...
          <button onClick={fetchCommissions} className="mt-4 px-4 py-2 text-sm font-medium rounded-[var(--radius-md)] transition-all duration-150 active:scale-[0.97] text-white"
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center" style={{ color: 'var(--color-text-muted)' }}>
          ...
          <p className="text-xs mt-1">{statusFilter !== 'all' ? 'Filter anpassen' : 'Noch keine Affiliate-Buchungen erstellt'}</p>
        </div>
      ) : (
            ...
            <thead>
              <tr style={{ backgroundColor: 'var(--color-shell-bg)' }}>
                {['ID', 'Affiliate', 'Vendor', 'Booking', 'Betrag', 'Rate', 'Status', 'Datum', 'Aktionen'].map(h => (
                  <th key={h} className="px-4 py-3 text-left font-medium text-xs uppercase tracking-wider" style={{ color: 'var(--color-text-muted)' }}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                return (
                  <tr key={c.id} className="transition-colors duration-150 hover:opacity-90"
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: st.bg, color: st.color }}>
                        {c.status}
                      </span>
                    </td>
                      ...
                      <div className="flex gap-1.5">
                        {c.status === 'pending' && (
                          <>
                            ...
                            <button onClick={() => handleAction(c.id, 'cancel')} disabled={actionLoading === c.id}
                              className="px-2.5 py-1 text-xs font-medium rounded-[var(--radius-sm)] transition-all duration-150 active:scale-[0.95] disabled:opacity-50 text-white"
